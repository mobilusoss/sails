...
module.exports = function(sails) {
  ...
  return function load(configOverride, cb) {
    ...
    async.auto({
      ...
      config: [Configuration.load.bind(Configuration)],
      ...
      // usage of the term.
      registry: ['hooks',
        function populateRegistry(async_data, cb) {
   ...
   * @api private
   */
  function loadHooks(async_data, cb) {
    sails.hooks = { };
  ...
  function loadHookDefinitions(hooks, cb) {
    ...
    if (sails.config.loadHooks) {
      ...
      _.each(hooks, function(def, hookName) {
        if (!_.includes(sails.config.loadHooks, hookName)) {
