    ...
    return {
      ...
      publish: function (models, eventName, context, data, req) {
        ...
        // Otherwise publish to each instance room
          ...
          var ids = _.map(models, this.primaryKey);
          if ( ids.length === 0 ) {
      ...
      subscribers: function (id, context) {
        ...
        id = id[self.primaryKey] || id;

        // For a single context, return just the socket subscribed to that context
        if (context) {
          return sails.sockets.subscribers(self.room(id, context), true);
        }
        ...
        var contexts = self.getAllContexts();
        var subscribers = [];
        _.each(contexts, function(context) {
          subscribers = _.union(subscribers, self.subscribers(id, context));
        });
      ...
      */
      subscribe: function (req, records, contexts) {
        ...
        var ids = _.map(records, this.primaryKey);
        ...
        if (options.previous && !options.noReverse) {
          ...
          _.forEach(changes, function(val, key) {
            ...
            if (association.type == 'model') {
              ...
              reverseAssociation = _.find(ReferencedModel.associations, {collection: self.identity, via: key}) || _.find(ReferencedModel.associations, {model: self.identity, via: key});
              ...
              reverseAssociation = _.find(ReferencedModel.associations, {collection: self.identity, via: key}) || _.find(ReferencedModel.associations, {model: self.identity, alias: association.via});
              ...
              if (!reverseAssociation) {return;}
                ...
                var previousPKs = _.map(previous[key], ReferencedModel.primaryKey);
        ...
        if (options.previous) {
          ...
          _.each(this.associations, function(association) {
            ...
            if (association.type == 'model' && association.alias && previous[association.alias]) {
              ...
              // Get the inverse association definition, if any
              reverseAssociation = _.find(ReferencedModel.associations, {collection: self.identity}) || _.find(ReferencedModel.associations, {model: self.identity});
