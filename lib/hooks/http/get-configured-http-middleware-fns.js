    ...
    session: (function() {
      if (sails.config.session && sails.config.session.store) {
        ...
        return function (req, res, next){
          ...
          var isSessionDisabled = _.some(sails.hooks.session.routesDisabled, function (disabledRouteInfo){

            // Figure out if the request's method matches.
            var isMethodExactMatch = req.method === disabledRouteInfo.method;
            var isMethodImplicitMatch = disabledRouteInfo.method === '' && _.includes(['GET', 'POST', 'PUT', 'DELETE', 'PATCH'], req.method);
